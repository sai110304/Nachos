...
int PCB::Exec(char* filename, int id) {
    ...
    // this->thread->parentThread=kernel->currentThread;
...
void StartProcess_3(void* pid) {
    int id;
    id = *((int*)pid);
    // Lay fileName cua process id nay
    // char* fileName = kernel->pTab->GetFileName(id);
    AddrSpace* space;
    AddrSpace* sp = kernel->currentThread->parentThread->space;
    space = new AddrSpace(sp);
...
int PCB::Exec2(char* filename, int id) {
    ...
    this->thread->Fork(StartProcess_3, &this->thread->processID);
    multex->V();
    // Trả về id.
    return id;
}
